...
public class RecordGesture : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(handInitializer.debugMode == false) { return; }

        if (handInitializer.isInitialized && Input.GetKeyDown(KeyCode.D))
        {
            GestureSnapshot();
        ...
        if (handInitializer.isInitialized && Input.GetKeyUp(KeyCode.D))
        {
        ...
        // so you can paste all the values in AFTER RUNTIME.
        if (handInitializer.isInitialized && Input.GetKeyDown(KeyCode.Space))
        {
            GestureSnapshot();
        }
    }
